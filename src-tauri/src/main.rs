...
fn main() {
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Shortcuts occasionally vanish silently (external keyboard hooks,
            // plugin quirks); probe periodically and re-register what is gone.
            hotkey::spawn_shortcut_health_check();
            // Resident translate window: build it hidden now so the first
            // hotkey press shows a warm webview instead of paying the
            // WebView2 spawn cost.
            window::spawn_precreate_translate_window();
            Ok(())
        })
