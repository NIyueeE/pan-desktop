...
#[tauri::command(async)]
#[cfg(target_os = "linux")]
pub fn system_ocr(_app_handle: tauri::AppHandle, _lang: &str) -> Result<String, String> {
    // Linux has no OS-bundled OCR engine; tesseract (the old CLI fallback)
    // was removed together with the Tesseract.js service in favour of the
    // bundled PaddleOCR engine. The recognize chain degrades to the next
    // instance (Paddle → system → VLM), so on Linux this errors honestly.
    Err("System OCR is not available on this platform; use the PaddleOCR service".to_string())
}
