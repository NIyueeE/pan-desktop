<script lang="ts">
    import { ArrowRightLeft, ChevronDown, ChevronUp, Copy, GripVertical, RefreshCw, Volume2 } from '@lucide/svelte';
    ...
    let result = $state('');
    // The target language the current result was produced with (the speak
    // button reads it — per-card, since second_language can redirect).
        ...
        service
            ...
            .then(
                (value: string) => {
                    ...
                    if (target === '') {
                        // An empty payload is a failure, not a blank card:
                        // surface it so the user sees why nothing appeared.
                        setError(t('translate.empty_result'));
                        isLoading = false;
                        return;
                    }
    ...
    $effect(() => {
        ...
        // Recommitted identical text must re-run the services.
        const epoch = translateState.commitEpoch;
        ...
        // Any source/language change invalidates in-flight attempts: a late
        // resolve from a superseded source must never repaint the card.
        generation += 1;
        result = '';
        errorText = '';
        if (text.trim() === '' || !from || !to) {
            // No content: fold back to the header instead of showing an
            // empty body.
            hidden = true;
            return;
        }
    ...
    function onRetry(): void {
        // Nothing committed: there is nothing to retry against.
        if (translateState.sourceText.trim() === '') {
            return;
        }
        errorText = '';
        result = '';
            ...
            <div class={`flex px-[12px] py-[5px] ${hidden ? 'hidden' : ''}`}>
                <div class="flex gap-[2px]">
                    ...
                    <button
                        ...
                        disabled={result === ''}
                        onclick={() => {
                            void speakText(result, spokenLanguage).catch((e: unknown) => {
