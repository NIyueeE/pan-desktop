<script lang="ts">
    import { Blocks, CloudUpload, Info, Keyboard, Languages, LayoutGrid, X, Minus, Square } from '@lucide/svelte';
    ...
    void trackConfigKeys(['app_font', 'app_font_size']);
...
<div class="flex h-screen w-screen {isLinux ? 'rounded-[10px] border border-default-100' : ''}">
    <nav
        class="float-left h-screen w-[230px] shrink-0 border-r border-default-100 select-none bg-content1 {isLinux
            ? 'rounded-l-[10px]'
            : ''}"
    >
    ...
    <main class="h-screen w-full select-none bg-content1 {isLinux ? 'rounded-r-[10px]' : ''}">
