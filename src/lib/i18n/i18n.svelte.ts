...
type AppLanguage = keyof typeof APP_LANGUAGES;
...
/** Languages matched by their exact ISO-639 primary subtag. */
const EXACT_LANGUAGE_TAGS: Partial<Record<string, AppLanguage>> = {
    en: 'en',
    ja: 'ja',
    ko: 'ko',
    fr: 'fr',
...
/**
 * Map a system locale (BCP-47 tag from plugin-os `locale()`, e.g. `zh-CN`)
 * onto a supported display language. Used once on first run to pick the
 * default; anything unrecognized falls back to English.
 */
export function matchAppLanguage(locale: string | null | undefined): AppLanguage {
    if (!locale) {
        return 'en';
    }
    const [lang = '', region = ''] = locale.trim().toLowerCase().replace(/-/g, '_').split('_');
    if (lang === 'zh') {
        // Traditional-script regions default to zh_tw; everything else
        // (including a bare `zh`) to the simplified variant.
        return /^(tw|hk|mo|hant)/.test(region) ? 'zh_tw' : 'zh_cn';
    }
    if (lang === 'pt') {
        return region.startsWith('br') ? 'pt_br' : 'pt_pt';
    }
    if (lang === 'nn') {
        return 'nn_no';
    }
    ...
    return EXACT_LANGUAGE_TAGS[lang] ?? 'en';
}
