...
export async function initConfigStore(): Promise<void> {
    ...
    readyPromise = (async () => {
        ...
        // prevent the whole UI from booting (legacy invariant). On a fresh
        // install config.json does not exist yet and a file watch fails —
        // fall back to watching the directory and filter for our file.
        try {
            await watch(path, () => {
            ...
            try {
                await watch(dir, (event) => {
                    if (event.paths.some((p) => p === path)) {
                        void onConfigFileChanged();
                    }
                });
            } catch (dirError) {
                void logWarn(`Failed to watch config file: ${dirError}`).catch(() => {});
            }
        }
    })();
    return readyPromise;
...
export function migrateLegacyKeys(): void {
    migrateLayoutKey();
    migrateTransparentKey();
}

function migrateLayoutKey(): void {
...
function migrateTransparentKey(): void {
    // `transparent: true` (legacy) already matches the 85% default — nothing
    // to write. An explicit opt-out (false) meant an opaque window.
    if (hasConfig('translate_opacity')) {
        return;
    }
    if (cfgRaw(LEGACY_TRANSPARENT_KEY) === false) {
        setConfig('translate_opacity', 100);
    }
}
