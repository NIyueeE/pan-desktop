...
const handlers: Record<string, (args: InvokeArgs) => unknown> = {
    ...
    // Dictionary URLs get a canned payload instead, so the translate
    // window's dictionary card can be previewed without network.
    'plugin:http|fetch': (args) => {
        const rid = ++nextResourceId;
        // plugin-http nests the request under `clientConfig`.
        const clientConfig = (args?.clientConfig ?? {}) as InvokeArgs;
        const url = str(clientConfig, 'url');
        if (url.includes('dict.youdao.com/jsonapi')) {
            const word = decodeURIComponent(/[?&]q=([^&]*)/.exec(url)?.[1] ?? 'hello');
            httpBodies.set(rid, {
                status: 200,
                body: new TextEncoder().encode(JSON.stringify(dictionaryPayload(word))),
            });
        } else if (url.includes('wiktionary.org/api/rest_v1/page/definition')) {
            const word = decodeURIComponent(url.split('/').pop() ?? 'hello');
            httpBodies.set(rid, {
                status: 200,
                body: new TextEncoder().encode(
                    JSON.stringify({
                        en: [
                            {
                                partOfSpeech: 'noun',
                                definitions: [{ definition: `An utterance of "${word}".`, examples: [] }],
                            },
                        ],
                    })
                ),
...
/** Canned Youdao jsonapi payload (ec dict + bilingual sentences) for
 * previews — same shape the real endpoint returns. */
function dictionaryPayload(word: string): unknown {
    return {
        ec: {
            word: [
                {
                    usphone: 'hello',
                    ukphone: 'həˈləʊ',
                    usspeech: `https://dict.youdao.com/dictvoice?audio=${encodeURIComponent(word)}&type=2`,
                    trs: [
                        { tr: [{ l: { i: ['int. 喂；哈罗，你好；您好（表示问候或引人注意）'] } }] },
                        { tr: [{ l: { i: ['n. 「复数」hellos：打招呼；问候（ helloes / hellos ）'] } }] },
                    ],
                },
            ],
        },
        blng_sents_part: {
            'sentence-pair': [
                {
                    sentence: 'Hello, everyone. Welcome to the show.',
                    'sentence-translation': '大家好，欢迎收看本期节目。',
                },
                {
                    sentence: 'She said hello to me in the corridor.',
                    'sentence-translation': '她在走廊里跟我打了招呼。',
                },
            ],
        },
    };
